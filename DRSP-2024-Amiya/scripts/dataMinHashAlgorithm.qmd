...
However, we now realize that we are somewhat comparing apples to oranges because our basic algorithm uses cosine similarity and MinHash uses Jaccard similarity. Let us try using a different LSH algorithm called SimHash that uses cosine similarity unlike MinHash.
