...
normalize_vector <- function(vec) {
  sum_vec <- sum(vec)
  if (sum_vec == 0) return(vec)  # Avoid division by zero
  vec / sum_vec
}
...
for (x in unique(small_data$article_id)) {
  ...
  article_vector <- normalize_vector(article_vector)
  ...
  for (i in seq_len(num_articles)) {
    for (j in seq_len(num_articles)) {
      if (i <= j) {
        ...
        dist <- hamming_distance(simhash_list[[i]], simhash_list[[j]])
        
        # Convert distance to similarity score
        sim <- 1 - dist / hash_size
        
        # Populate the matrix
        simHash_similarity_matrix[i, j] <- sim
        simHash_similarity_matrix[j, i] <- sim
      }
    }
  }
...
plot_data <- data.frame(
  Algorithm = c("Algorithm 1", "Algorithm 2"),
  Accuracy = c(accuracy_cosine, accuracy_simHash),  # Example accuracy values
  Time = c(15000, 760)
)
...
ggplot(plot_data, aes(x = Algorithm, y = Accuracy, fill = Algorithm)) +
  ...
  scale_fill_manual(values = c("Algorithm 1" = "blue", "Algorithm 2" = "red")) +
...
# Alternatively, create a point plot
ggplot(data, aes(x = Algorithm, y = Accuracy, color = Algorithm)) +
  geom_point(size = 5) +
  geom_segment(aes(x = Algorithm, xend = Algorithm, y = 0, yend = Accuracy), size = 1) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  scale_color_manual(values = c("Algorithm 1" = "blue", "Algorithm 2" = "red")) +
  labs(title = "Accuracy Comparison of Similarity Search Algorithms",
       x = "Algorithm",
       y = "Accuracy") +
  theme_minimal() +
