...
We can try to clean our data by removing duplicate articles for the article pairs with the top 10 similarity scores.

```{r}
# Flatten the matrix to a vector for easier sorting
flattened_vector <- as.vector(rounded_matrix)

# Find the top 10 maximum similarity values
top_10_values <- sort(flattened_vector[flattened_vector != 1], decreasing = TRUE)[1:10]

# Display the top 10 similarity values
top_10_values

# Find the indices of the top 10 similarity values in the matrix
top_10_indices <- list()
for (value in top_10_values) {
  indices <- which(rounded_matrix == value, arr.ind = TRUE)
  top_10_indices[[as.character(value)]] <- indices
}

# Display the indices of the top 10 similarity values
top_10_indices
```

Graph the similarity scores
