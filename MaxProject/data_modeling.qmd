...
kcca_with_progress <- function(data, k, dist_function = "kmeans", nstart = 1) {
  ...
  for (i in 1:nstart) {
    set.seed(i)
    kcca_result <- kcca(data, k = k, family = kccaFamily(which = dist_function), control = list(initcent = "kmeanspp"))
...
```{r}
new_to_old_mapping <- hash(keys = mapping$new_id, values = mapping$original_id)
```
...
```{r}
kcca_result <- kcca_with_progress(reduced_matrix, k = 100, "angle", nstart = 20)
kcca_result
```
...
cluster_data <- data.frame(
  ...
  original_id = sapply(1:length(kcca_result@cluster), function(x) {
    new_to_old_mapping[[as.character(x)]]
  })
)
...
for (cluster_num in 1:100){
  ...
  #calculate aggregate tfidf for each word in the cluster. (Finds most important words)
  ...
  print(aggregated_tf_idf)
}
```

```{r}
df1 <- data.frame(col1 = c(1, 2, 3), col2 = c(10, 20, 30), ignore = c(100, 200, 300))
mutate(rowwise(df1), total_sum = sum(c_across(starts_with("col")), na.rm = TRUE))
```
