...
nyt_vocab_vector <- setNames(1:nrow(nyt_vocab), nyt_vocab$word)
...
for (i in 1:length(valid_word_ids)) {
  word <- names(nyt_vocab_vector)[i]
  if (!is.null(english_vocab_set[[ word ]] ) || startsWith(word, 'zzz')) {
...
for (x in unique(small_data$article_id)) {
  ...
  for (i in 1:nrow(article_data)) {
    ...
    if(valid_word_ids[word_index]) {
      article_vector[word_index] <- word_occurences
    }
  }
